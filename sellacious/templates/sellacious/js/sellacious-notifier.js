...
jQuery(document).ready(function ($) {
	var JARVIS_SITE = '//sellacious.com';
	...
	SellaciousNotifier.prototype = {
		...
		fetch: function (callback) {
			$.ajax({
					url: JARVIS_SITE + '/index.php?option=com_jarvis&view=views&format=json',
					type: 'POST',
					dataType: 'json',
					cache: false,
					data: {
		...
		display: function (banners) {
			...
			if (banners.length)
				$(banners).each(function (index, banner) {
					banner.link = JARVIS_SITE + '/index.php?option=com_banners&task=click&id=' + banner.id;
		...
		image: function (banner) {
			...
			if ($element.length) {
				$('<img/>').load(function () {
					...
					src: JARVIS_SITE + '/' + banner.params['imageurl'],
