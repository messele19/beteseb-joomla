...
class SellaciousControllerTransaction extends SellaciousControllerForm
{
	...
	protected function allowAdd($data = array())
	{
		...
		$type  = $app->getUserState("$this->option.edit.$this->context.type");
		$allow = false;
	...
	public function add()
	{
		...
		$app->setUserState("$this->option.edit.$this->context.type", 'addfund');

		if (!parent::add())
		{
			$app->setUserState("$this->option.edit.$this->context.type", null);
	...
	public function withdraw()
	{
		...
		$app->setUserState("$this->option.edit.$this->context.type", 'withdraw');
	...
	public function setType()
	{
		...
		$app->setUserState("$this->option.edit.$this->context.data", $post);
		$this->setRedirect(
			JRoute::_(
	...
	public function initPayment($txn_id = null)
	{
		...
		try
		{
			$txn_id      = $txn_id ? $txn_id : $app->input->getInt('id');
			$transaction = $this->helper->transaction->getItem($txn_id);
			...
			$app->setUserState('com_sellacious.payment.execution.params', $transaction->payment_params);
