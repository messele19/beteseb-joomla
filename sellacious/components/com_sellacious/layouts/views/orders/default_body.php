...
JHtml::_('script', 'com_sellacious/util.modal.js', true, true);
JHtml::_('script', 'com_sellacious/view.orders.js', true, true);

JHtml::_('stylesheet', 'com_sellacious/component.css', null, true);
JHtml::_('stylesheet', 'com_sellacious/view.orders.css', null, true);
...
foreach ($this->items as $i => $order)
{
	...
	<tr id="order-<?php echo $i ?>" class="order-row">
		<td class="center hidden-phone" style="width:40px">
			<input type="checkbox" name="cid[]" id="cb<?php echo $i ?>" class="hidden" value="<?php echo $order->id ?>" title=""/>
		...
		<td class="text-left" data-row="#order-items-<?php echo $i ?>">
			...
			{
				?>
				<button type="button" class="btn-toggle button-mini pull-right"><?php echo JText::_('COM_SELLACIOUS_ORDER_EXPAND'); ?></button>
				<button type="button" class="btn-toggle button-mini pull-right hidden"><?php echo JText::_('COM_SELLACIOUS_ORDER_COLLAPSE'); ?></button>
				<?php
			}
			?>
		</td>
