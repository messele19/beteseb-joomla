...
JHtml::_('script', 'com_sellacious/view.ratings.js', false, true);
JHtml::_('stylesheet', 'com_sellacious/component.css', null, true);
JHtml::_('stylesheet', 'com_sellacious/view.ratings.css', null, true);
...
foreach ($this->items as $i => $item)
{
	...
	$image_url   = $this->helper->media->getImage('products', $item->product_id, true);
	?>
	<tr>
		...
		<td>
			...
			<?php if (strlen($comment)): ?>
				...
				<div class="static-modal hidden">
					...
					<strong><?php echo $this->escape(ucwords($item->author_name)); ?></strong> –
						<?php echo $stars ?>
					<hr class="standard">
					<strong><?php echo trim($item->title) ?></strong><br>
					<?php echo $comment; ?>
				</div>
			<?php endif; ?>
		</td>
