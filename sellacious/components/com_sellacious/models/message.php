...
class SellaciousModelMessage extends SellaciousModelAdmin
{
	...
	public function save($data)
	{
		...
		try
		{
			...
			$array = array(
				...
				'params'    => array('recipients' => $sel_cats),
			);
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		{
			$me    = JFactory::getUser();
			$owned = $item->get('sender') == $me->id || $item->get('recipient') == $me->id;
			$allow = $this->helper->access->check('message.reply') || ($owned && $this->helper->access->check('message.reply.own'));
		}
	...
	protected function processItem($item)
	{
		...
		{
			$this->setError(JText::_('COM_SELLACIOUS_PAGE_NOT_FOUND'));
		}
