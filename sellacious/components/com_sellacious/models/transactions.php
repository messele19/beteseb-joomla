...
class SellaciousModelTransactions extends SellaciousModelList
{
	...
	public function getFilterForm($data = array(), $loadData = true)
	{
		...
		if ($form instanceof JForm)
		{
			...
			else
			{
				...
				$currency->where('a.context = ' . $this->_db->q('user.id'))
					->where('a.context_id = ' . (int) JFactory::getUser()->id);
			}
	...
	protected function getListQuery()
	{
		...
		$query->select($db->qn('u.name', 'context_title'))
			->join('left', '#__users u ON u.id = a.context_id AND a.context = ' . $db->q('user.id'));
		...
		elseif ($this->helper->access->check('transaction.list.own'))
		{
			// Force user filter to limit to own items.
			$me = JFactory::getUser();
			$query->where('a.context = ' . $db->q('user.id'));
			$query->where('a.context_id = ' . (int) $me->id);
