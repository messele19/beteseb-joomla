...
class SellaciousModelProduct extends SellaciousModelAdmin
{
	...
	protected function preprocessData($context, &$data, $group = 'content')
	{
		...
		if ($this->helper->access->checkAny(array('seller', 'pricing', 'shipping'), 'product.edit.'))
		{
			...
			{
				$data->seller_uid = $app->getUserState('com_sellacious.edit.product.seller_uid', null);
			}
		}
		elseif ($this->helper->access->checkAny(array('seller.own', 'pricing.own', 'shipping.own'), 'product.edit.'))
		{
			$data->seller_uid = $multi_seller || $default_seller == $me->id ? $me->id : 0;
		}
		else
		{
			$data->seller_uid = 0;
		...
		$app->setUserState('com_sellacious.edit.product.seller_uid', $data->seller_uid);
	...
	protected function preprocessForm(JForm $form, $data, $group = 'sellacious')
	{
		...
		if ($product_type)
		{
			...
			if ($basicEdit)
			{
				...
				$form->setFieldAttribute('attachments', 'record_id', $product_id, 'basic');
				$form->setFieldAttribute('images', 'record_id', $product_id, 'basic');
			...
			if ($seller_uid)
			{
				...
				if ($sellerEdit)
				{
					...
					$form->setFieldAttribute('attachments', 'record_id', $registry->get('seller.id'), 'seller');
					...
					list($sh, $st, $sot) = $this->helper->product->getStockHandling($product_id, (array) $registry->get('categories'));
		...
		try
		{
			...
			if ($seller_uid = $registry->get('seller_uid'))
			{
				...
				if ($sellerEdit && ($attributes = $registry->extract('seller')))
				{
					...
					$this->helper->product->setSellerAttributesByType($attributes->toArray(), $product_id, $seller_uid, $type);
				}
	...
	public function duplicate($productId)
	{
		...
		if ($this->helper->config->get('free_listing'))
		{
			$filter = array('list.from' => '#__sellacious_seller_listing', 'product_id' => $productId);
			$items  = $this->helper->product->loadObjectList($filter);
	...
	public function removeEProductMedia($id)
	{
		...
		$product_id = $app->getUserState('com_sellacious.edit.product.id', null);
		$seller_uid = $app->getUserState('com_sellacious.edit.product.seller_uid', null);
