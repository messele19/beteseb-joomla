...
class SellaciousModelProductListing extends SellaciousModelAdmin
{
	...
	protected function populateState()
	{
		...
		/** @var array $pks */
		$pks = $app->getUserStateFromRequest('com_sellacious.productlisting.products', 'cid', array(), 'array');
		...
		{
			$userState = (object) $app->getUserState('com_sellacious.edit.productlisting.data', null);
			$app->setUserState('com_sellacious.edit.productlisting.data', $userState);

			if (!isset($userState->seller_uid))
			{
				$seller_uid = $app->getUserState('com_sellacious.products.filter.seller_uid', null);
			}
			else
			{
		...
		$app->setUserState('com_sellacious.edit.productlisting.data.seller_uid', $seller_uid);
	...
	public function validate($form, $data, $group = null)
	{
		...
		$wallet_bal = $this->helper->transaction->getBalance($seller_uid, $currency);
		$fees       = array_sum(ArrayHelper::getColumn($listings, 'fee_total'));
		$fee_total  = $fees * count($prods);

		if (empty($wallet_bal->amount) || $wallet_bal->amount < $fee_total)
		{
			$fee_d = $this->helper->currency->display($fee_total, $currency, null);
			$wb_d  = $this->helper->currency->display($wallet_bal->amount, $currency, null);
