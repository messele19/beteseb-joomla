...
class SellaciousModelTransaction extends SellaciousModelAdmin
{
	...
	public function validate($form, $data, $group = null)
	{
		...
		elseif ($type == 'withdraw')
		{
			...
			$balance  = $this->helper->transaction->getBalance($vData['user_id'], $currency);

			if (!$balance || round($balance->amount - $withdrawal['amount'], 2) < 0.00)
			{
				$bal_d = $balance ? $balance->display : $this->helper->currency->display(0, $currency, null);
				$amt_d = $this->helper->currency->display($withdrawal['amount'], $currency, null);

				$this->setError(JText::sprintf('COM_SELLACIOUS_TRANSACTION_INSUFFICIENT_WALLET_BALANCE', $amt_d, $bal_d));
	...
	public function save($record)
	{
		...
		$balance  = $this->helper->transaction->getBalance($record['user_id'], $currency);
